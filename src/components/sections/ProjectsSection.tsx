...
import { Github, ShieldCheck, Layers, ArrowUpRight, Sparkles, Cpu, Server, CheckCircle } from "lucide-react";
  ...
  return (
    <section id="projects" className="py-16 sm:py-24 relative bg-cream-100 dark:bg-dark-950 text-dark-900 dark:text-cream-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Touch Scrollable Category Tabs Wrapper */}
        <div className="w-full overflow-x-auto pb-4 mb-8 sm:mb-12 scrollbar-none [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="flex items-center justify-start sm:justify-center gap-2.5 min-w-max px-4 sm:px-6">
            {categories.map((cat) => (
              <button
                key={cat.id}
        ...
        {(selectedCategory === "all" || selectedCategory === "Cloud/DevOps") && (
          ...
          >
            <div className="p-6 sm:p-8 lg:p-10 rounded-3xl bg-white/95 dark:bg-dark-900/95 border-2 border-azure-500/40 dark:border-cyan-400/40 backdrop-blur-2xl shadow-xl relative overflow-hidden group hover:border-azure-500 dark:hover:border-cyan-400 transition-all duration-300">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-center">
                ...
                <div className="lg:col-span-5 flex flex-col gap-3 p-6 rounded-2xl bg-slate-50 dark:bg-dark-800/80 border border-slate-200 dark:border-slate-700/80 font-mono text-xs">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-700">
                    <span className="text-azure-600 dark:text-cyan-400 font-bold flex items-center gap-1.5">
                      <Cpu className="w-3.5 h-3.5" />
                      <span>IaC Framework</span>
                    </span>
        ...
        {/* Other Projects 3D Tilt Cards Grid */}
          ...
          {otherProjects.map((project, idx) => (
            ...
            >
              <TiltCard className="h-full">
                <div className="p-6 sm:p-8 rounded-3xl bg-white/95 dark:bg-dark-900/95 border border-slate-200 dark:border-slate-800 hover:border-azure-500/50 dark:hover:border-cyan-400/50 transition-all duration-300 shadow-lg h-full flex flex-col justify-between group">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xs font-mono font-bold text-azure-600 dark:text-cyan-400 uppercase tracking-wider">
                        {project.subtitle}
                      </span>
                    ...
                    <h3 className="font-heading font-extrabold text-xl sm:text-2xl text-dark-900 dark:text-white mb-3 group-hover:text-azure-600 dark:group-hover:text-cyan-300 transition-colors">
                      {project.title}
                    </h3>

                    <p className="text-dark-800/90 dark:text-cream-300/90 text-xs sm:text-sm font-body leading-relaxed mb-6 line-clamp-3">
                      {project.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {project.tags.map((tag) => (
                        <span
                          key={tag}
                          className="px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-dark-800 border border-slate-200 dark:border-slate-700 text-[11px] font-mono font-semibold text-dark-800 dark:text-slate-200"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <button
                      ...
                      className="text-xs font-bold font-heading text-azure-600 dark:text-cyan-300 hover:underline flex items-center gap-1"
                    >
